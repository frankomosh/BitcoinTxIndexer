...
use crate::{
    config::Config,
    db::{Database, models::{BlockModel, TransactionModel}},
    indexer::runes::RunesProcessor,
};
...
impl BitcoinIndexer {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let mut current_height = self.db.get_last_block_height()
            .await?
        ...
        loop {
            ...
            let end_height = (current_height + self.config.indexer_batch_size as u64)
                .min(latest_height);
    ...
    async fn process_transaction(
        ...
        // Store transaction
        self.db.insert_transaction(tx, block_height, &block.header).await?;
        
        // Check if this is a Runes transaction
        if let Some(runes_data) = self.runes_processor.process_transaction(tx)? {
            self.db.insert_runes_transaction(&runes_data, tx, block_height).await?;
        }
